<?php
    ...
    if(isset($_SESSION['username'])){//check if there is session with this username
        $pagetitle = 'المدرسين'; // set page title
        ...
        if($do == 'Manage'){//manage page
            ...
            ?>
            <h1 class="text-center"><?php echo lang('Manage Teachers')?></h1>
            <!-- start container -->
            <div class="container">
        ...
        <?php
            ...
            ?>
            <div class="container">
                <a href="?do=Add" class="add btn btn-lg btn-primary">
                    <span class="glyphicon glyphicon-plus"></span> <?php echo lang('Add New Teahcer')?></a>
                <form method="post" action="teacher_report.php" class="pdf form-horizontal">
        ...
        <?php
            ...
            if($count>0){//if number of row large than 0
                ?>
                    <h1 class="text-center"><?php echo lang('Edit Teacher');?></h1>
                    <!-- start the container -->
                    <div class="container">
                        <!-- start update form -->
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){//CEHCK that user coming through post request
                echo '<h1 class="text-center">'.lang('Update Teacher').'</h1>';
                echo '<div class="container">';
                ...
                if(strlen($teachername)<4){
                    $formerror[] = lang('Teacher Name Can Not Be Less Than 4 Character');
                }
            ...
            ?>
            <h1 class="text-center"><?php echo lang('Add Teacher');?></h1>
            <!-- start container -->
            <div class="container">
                <!-- start add form -->
                <form class="form-horizontal" action="?do=Insert" method="POST">
                    <!-- start teacher name -->
                    <div class="form-group form-group-lg">
                        <label class="control-label col-sm-2"><?php echo lang('Teacher Name')?></label>
                        <div class="col-sm-10 col-md-6 pull-right">
        ...
        <?php
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){//check that user coming through post request
                echo '<h1 class="text-center">'.lang('Insert Teacher').'</h1>';
                echo '<div class="container">';
                $teachername = $_POST['teachername'];//receive teacher name and store it in var
                ...
                if(empty($formerror)){//if formerror is empty execute query
                    ...
                    if($check == 1){//if check == 1 this mean that teacher is exist show this message
                        $msg = '<div class="alert alert-danger">'.lang('Sorry This Teacher Name is Exist').'</div>';
            ...
            echo '<h1 class="text-center">'.lang('Delete Teacher').'</h1>';
            echo '<div class="container">';
