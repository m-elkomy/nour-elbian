<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){//check if user coming through post request
        ...
        if($count>0){//if count > 0 this mean that this username and password exist so register and start session
            ...
            $_SESSION['GroupID']       = $row['GroupID']; //register session with User ID
...
?>
    ...
    <form class="login" action="<?php echo $_SERVER['PHP_SELF']?>" method="POST">
        ...
        <input type="text" name="user" placeholder="<?php echo lang('User Name')?>" autocomplete="off" class="form-control" />
        <input type="password" name="pass" placeholder="<?php echo lang('Password')?>" autocomplete="new-password" class="form-control" />
