<?php
    ...
    if(isset($_SESSION['username'])){//check if there is session with this username
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($branchid == 0) {
                function fetch_data()
                {
                    global $con;
                    $output = '';
                    $stmt = $con->prepare("SELECT teachers.*,branch.* FROM 
      ...
      <table align="center" border="1" cellspacing="0" cellpadding="5">  
           <tr>  
                <th style="background-color:#31B0D5;color:#fff" width="15%">'.lang('Teacher ID').'</th>  
                <th style="background-color:#31B0D5;color:#fff" width="40%">'.lang('Teacher Name').'</th>  
      ...
      ';
                ...
                function fetch_data($branchid)
                {
                    global $con;
                    $output = '';
                    $stmt = $con->prepare("SELECT teachers.*,branch.* FROM teachers 
                                                    INNER JOIN branch 
                                                    ON teachers.BranchID = branch.BranchID
