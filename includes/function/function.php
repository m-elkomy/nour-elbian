...
function calcsalary($salary,$dyasofabsence){
    $calc = $salary - ($dyasofabsence * 30);
    return $calc;
}


function checkda($table,$val1,$val2){
    global $con;
    $stmt = $con->prepare("SELECT Date FROM $table WHERE Date = ? AND TeacherID = ?");
    $stmt->execute(array($val1,$val2));
    $count = $stmt->rowCount();
    return $count;
}

function calc($teacherid){
    $month = date('m');
    $year  = date('y');
    $day   = date('d');
    $from  = date($year .'-' . $month. '-' .'1');
    $to    = date($year .'-' . $month. '-' .'30');
    global $con;
    $statement = $con->prepare("SELECT COUNT(absence.Date) AS days FROM absence INNER JOIN 
                                                        teachers ON absence.TeacherID = teachers.TeacherID 
                                                        WHERE teachers.TeacherID = ?
                                                        AND absence.Date between ? AND ?");
    $statement->execute(array($teacherid,$from,$to));
    $rows = $statement->fetchColumn();
    if($rows == 0){
        $stmt = $con->prepare("SELECT salary.Salary FROM salary INNER JOIN teachers 
                                    ON salary.TeacherID = teachers.TeacherID WHERE teachers.TeacherID=?");
        $stmt->execute(array($teacherid));
        $arow = $stmt->fetchColumn();
        $totalsalary = ($arow);
        return $totalsalary;
    }elseif($rows!=0){
        $stmt = $con->prepare("SELECT salary.Salary FROM salary INNER JOIN teachers 
