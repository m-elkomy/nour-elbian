<?php
    ...
    if(isset($_SESSION['username'])){//if session is register with username
        $pagetitle = 'الفصول'; // set page title
        ...
        if($do == 'Manage'){ // manage page
            ...
            ?>
            <h1 class="text-center"><?php echo lang('Manage Classes')?></h1>
            <!-- start container -->
            <div class="container">
        ...
        <?php
                ...
                echo '<div class="container">
                       <div class="nice-message alert alert-info">'.lang('Sorry There Is No Recored To Show').'</div>
                    </div>';
            }?>
            <div class="container"><a href="?do=Add" class="add btn btn-lg btn-primary">
                    <span class="glyphicon glyphicon-plus"></span>
                    <?php echo lang('Add New Class')?></a>
                <form method="post" action="class_report.php" class="pdf form-horizontal">
        ...
        ?>
            <h1 class="text-center"><?php echo lang('Edit Class')?></h1>
            <!-- start container -->
            <div class="container">
                <!-- start edit form -->
                <form class="form-horizontal" action="?do=Update" method="POST">
                    ...
                    <!-- start class name -->
                    <div class="form-group form-group-lg">
                        <label class="control-label col-sm-2"><?php echo lang('Class Name')?></label>
                        <div class="col-sm-10 col-md-6 pull-right">
                            <input type="text" class="form-control" name="cname"
                                   placeholder="<?php echo lang('Class Name')?>" autocomplete="off"
        ...
        <?php
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){//check that user coming through post request
                echo '<h1 class="text-center">'.lang('Update Class').'</h1>';
                ...
                if(empty($formerror)){//if formerror array is empty
                    ...
                    //success message
                    $msg = '<div class="alert alert-success">' . $stmt->rowCount() . ' ' . lang('Row Updated'). '</div>';
                    redirect($msg,'back');//redirect to previous page
                }
                echo '</div>';
            }else{
                //if user not coming to this page using post request show this message
                $msg = '<div class="alert alert-danger">'.lang('Sorry You Can Not Browse This Page Direclty').'</div>';
                redirect($msg);//redirect the user to login page
            }
        }elseif($do == 'Add'){//add page ?>
            <h1 class="text-center"><?php echo lang('Add Class')?></h1>
            <!-- start container -->
            <div class="container">
                ...
                <form class="form-horizontal" action="?do=Insert" method="POST">
                    <!-- start class name -->
                    <div class="form-group form-group-lg">
                        <label class="col-sm-2 control-label"><?php echo lang('Class Name')?></label>
                        <div class="col-sm-10 col-md-6 pull-right">
                            <input type="text" name="cname" class="form-control"
                                   placeholder="<?php echo lang('Class Name')?>" autocomplete="off" required="required"/>
                        </div>
                    </div>
                    <!-- end class name -->
                    <!-- start branch name -->
        ...
        <?php
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){// check that user coming through post request
                echo '<h1 class="text-center">'.lang('Insert Class').'</h1>';
                ...
                if(empty($formerror)){//if formerror array is empty
                    ...
                    if($check == 0){//if not exist
                        ...
                        $msg = '<div class="alert alert-danger">'.lang('Sorry This Class Name Is Exist').'</div>';
            ...
            echo '<h1 class="text-center">'.lang('Delete Class').'</h1>';
            echo '<div class="container">';//start the container
