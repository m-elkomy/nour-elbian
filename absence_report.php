        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($branchid == 0) {
                function fetch_data()
                {
                    global $con;
                    $output = '';
                    $stmt = $con->prepare("SELECT DISTINCT teachers.*,branch.*,absence.* FROM 
                                                    teachers INNER JOIN branch 
                                                    ON teachers.BranchID = branch.BranchID
                                                    INNER JOIN absence ON teachers.TeacherID = 
                                                    absence.TeacherID
                ...
                if (isset($_POST["create_pdf"])) {
                    ...
                    $content = '';
                    $content .= '  
      <h3 align="center">'.lang('Absence').'</h3><br /><br />  
      <table align="center" border="1" cellspacing="0" cellpadding="5">  
           <tr>  
                <th style="background-color:#31B0D5;color:#fff" width="10%">'.lang('Teacher ID').'</th>  
                <th style="background-color:#31B0D5;color:#fff" width="30%">'.lang('Teacher Name').'</th>  
                <th style="background-color:#31B0D5;color:#fff" width="30%">'.lang('Branch Name').'</th>  
                <th style="background-color:#31B0D5;color:#fff" width="30%">'.lang('Date').'</th>   
           </tr>  
      ';
                ...
                function fetch_data($branchid)
                {
                    global $con;
                    $output = '';
                    $stmt = $con->prepare("SELECT teachers.*,branch.*,absence.* 
                                                    FROM teachers INNER JOIN branch 
                                                    ON teachers.BranchID = branch.BranchID 
                                                    INNER JOIN absence ON teachers.TeacherID = 
                                                    absence.TeacherID 
                                                    WHERE teachers.BranchID = ? 
                                                    ORDER BY teachers.TeacherID ASC");
